...
struct S {
  S() = default;
  explicit S(const std::pmr::vector<unsigned char> &vec, unsigned long value)
      : a(vec), b(value) {}
...
int main() {
  ...
  std::cout << "Sequence: " << std::fixed << std::setprecision(3)
  ...
  std::cout << "Composite: " << std::fixed << std::setprecision(3)
            << elapsed_seconds.count() << "us\n";
  ...
  elapsed_seconds = end - start;
  std::cout << "Composite Normal: " << std::fixed << std::setprecision(3)
            << elapsed_seconds.count() << "us\n";
